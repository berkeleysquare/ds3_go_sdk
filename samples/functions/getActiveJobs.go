...
func GetAciveJobsSample() {
    fmt.Println("---- Get Active Jobs Sample ----")
    ...
    request := models.NewGetActiveJobsSpectraS3Request()

    // Perform the Get Service call by using the client and invoking the desired command.
    response, err := client.GetActiveJobsSpectraS3(request)
    if err != nil {
        log.Fatal(err)
    }
    ...
    fmt.Println("Buckets:")
    for i, job := range response.ActiveJobList.ActiveJobs {
        fmt.Printf("%d) Name: %s; Bucket: %s\n", i + 1, utils.ToSafeString(job.Name), job.BucketId)
    }
}
